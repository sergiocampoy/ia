...
Action ComportamientoJugador::think(Sensores sensores) {
	...
	if (sensores.nivel != 4){
		...
		// Espera a que el aldeano se aparte
		if (sensores.superficie[2] == 'a' and plan.front() == actFORWARD) {
			return actIDLE;
		}
		// Espera mientras se está cargando
...
bool ComportamientoJugador::pathFinding (int level, const estado &origen, const estado &destino, list<Action> &plan){
	switch (level){
		...
		case 4: cout << "Busqueda para el reto\n";
						return pathFinding2(origen, destino, plan);
						break;
	}
...
bool ComportamientoJugador::pathFinding2(const estado &origen, const estado &destino, list<Action> &plan) {
   ...
   nodo current;
	...
	/*
	printf("Destino: (%d, %d)", destino.columna, destino.fila);
	for (unsigned i = 0; i < 30; i++) {
		for (unsigned j = 0; j < 30; j++) {
			printf("%c ", mapaResultado[i][j]);
		}
		printf("\n");
	}
	*/
  ...
  while (!cola.empty() and (current.st.fila!=destino.fila or current.st.columna != destino.columna)){
		...
		unsigned char casilla = mapaResultado[current.st.fila][current.st.columna];
		estado tmp;
		tmp.fila = current.st.fila;
		tmp.columna = current.st.columna;
		unsigned costo; // = distance(tmp, destino);
		switch (casilla) {
			case 'A':
